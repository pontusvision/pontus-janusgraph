...
abstract public class JanusGraphAbstractAuthenticator implements Authenticator
{
    ...
    // LPPM - upgrade to Tinkerpop 3.4.0 CredentialGraph no longer exists....
    protected CredentialTraversalSource credentialStore;
    ...
    @Override abstract public SaslNegotiator newSaslNegotiator(final InetAddress remoteAddress);

    public CredentialTraversalSource createCredentialGraph(JanusGraph graph)
    {
        return new CredentialTraversalSource(graph);
    }
    ...
    @Override public void setup(final Map<String, Object> config)
    {
        ...
        Preconditions.checkArgument(config != null, String
            .format("Could not configure a %s - provide a 'config' in the 'authentication' settings",
                this.getClass().getName()));

        Preconditions.checkState(config.containsKey(CONFIG_CREDENTIALS_DB), String
            .format("Credential configuration missing the %s key that points to a graph config file or graph name",
                CONFIG_CREDENTIALS_DB));

        if (!config.containsKey(CONFIG_DEFAULT_USER) || !config.containsKey(CONFIG_DEFAULT_PASSWORD))
        {
            logger.warn(String.format("%s and %s should be defined to bootstrap authentication", CONFIG_DEFAULT_USER,
        ...
        {
            final PropertyKey username = mgmt.makePropertyKey(PROPERTY_USERNAME).dataType(String.class)
            ...
            if (!index.getIndexStatus(username).equals(SchemaStatus.ENABLED))
            {
                try
                {
                    ...
                    ManagementSystem.awaitGraphIndexStatus(graph, USERNAME_INDEX_NAME).status(SchemaStatus.ENABLED)
                        .call();
                    mgmt.commit();
                }
                catch (InterruptedException rude)
                {
                    ...
                    throw new RuntimeException(
                        "Timed out waiting for byUsername index to be created on credential graph", rude);
                }
            }
        }

        final String defaultUser = config.get(CONFIG_DEFAULT_USER).toString();
        // LPPM - upgrade to Tinkerpop 3.4.0
        credentialStore.users(defaultUser, config.get(CONFIG_DEFAULT_PASSWORD).toString());
        //        if (credentialStore.findUser(defaultUser) == null) {
        //            credentialStore.createUser(defaultUser, );
        //        }
    }
}
