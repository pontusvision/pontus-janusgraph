    ...
    CONTAINS_REGEX
        {
            ...
            @Override public boolean evaluateRaw(String value, String regex)
            {

                //            for (String token : tokenize(value.toLowerCase())) {
                // LPPM - attempt to fix an issue where calling containsRegex() multiple
                // times in the same query can cause values not to be returned.
